...
-- ── second query, run it too ───────────────────────────────────────────────
-- On a database where the migrations have never been run this one errors with
-- "relation public.app_admins does not exist". That is the correct answer, not
-- a fault in the check: the query above has already reported the table as
-- missing, and this one has nothing to count.
...
--
-- It is a separate statement rather than another arm of the union above
-- because Postgres binds every table in a statement when it PLANS it, not when
-- a branch is taken. Guarding it with "where exists (... information_schema
-- ...)" reads like it protects the reference, and does not: on a database
-- where app_admins has not been created yet the whole union failed to parse,
-- so the check that was meant to list everything missing returned nothing at
-- all. Same shape as the EXECUTE-is-checked-at-prepare-time note in
