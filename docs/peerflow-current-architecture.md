...
Routes are files, so URLs carry `.html`. `vercel.json` rewrites exactly one
path (`/draft` to `draft.html`) and there is no catch-all: anything else that
matches no file is a 404, served from `404.html`.
